...
// = 038 ======================================================================
// シェーダを使って、まるで新聞やポスターのような、ハーフトーン調のシェーディン
// グを行なってみましょう。
// モザイクの場合もそうでしたが、実際にはかなり単純な計算なんですがどうしても混
// 乱しやすい処理がシェーダで行われます。図解したり、式を書いてみたり、とにかく
// 一番単純に考えたらどうなるかを落ち着いて想像しながら取り組みましょう。
// ============================================================================
...
(() => {
    ...
    let toneRatioSlider;      // スライダー GUI（トーンの大きさ係数） @@@
    let toneRatio = 1.0;      // トーンの大きさ係数 @@@
    ...
    window.addEventListener('load', () => {
        ...
        // トーンの大きさ係数 @@@
        toneRatioSlider = new gl3.Gui.Slider('tone ratio', toneRatio, 1.0, 10.0, 0.01);
        toneRatioSlider.add('input', (eve, self) => {
            toneRatio = self.getValue();
        });
        wrapper.append(toneRatioSlider.getElement());
