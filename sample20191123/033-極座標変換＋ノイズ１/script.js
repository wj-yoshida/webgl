...
/** ===========================================================================
 * これまで、テクスチャ座標には一貫して直交座標系を用いてきました。
 * 直交座標系とは、X 軸と Y 軸が直交する、というように、軸同士が直交する座標の表
 * 現のことです。これに対して、極座標系という考え方もあります。
 * 極座標系では、座標は角度と距離（半径）によって表現されます。このような座標系
 * を使うことにどんな意味があるのか最初はちょっとわかりにくいかもしれませんが、
 * 数学的な意味はさておき、シェーダではどのように極座標が活用できるのか、今回の
 * 例を見ながら考えてみましょう。
 * ========================================================================= */
//テクスチャの色によって移動する距離が違う
const MAT = new matIV();
const QTN = new qtnIV();

let timeScale = 0.05;      // 時間の経過に対するスケール @@@
let distortionScale = 0.1; // 歪み係数 @@@
let polar = false;         // 極座標変換するかどうか @@@
let noiseVisible = false;  // ノイズを可視化するかどうか @@@
...
class WebGLFrame {
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.postUniLocation = [
                    ...
                    gl.getUniformLocation(this.postProgram, 'timeScale'),       // 時間の経過に対するスケール @@@
                    gl.getUniformLocation(this.postProgram, 'distortionScale'), // 歪み係数 @@@
                    gl.getUniformLocation(this.postProgram, 'polar'),           // 極座標変換するかどうか @@@
                    gl.getUniformLocation(this.postProgram, 'noiseVisible'),    // ノイズを可視化するかどうか @@@
                ];
                this.postUniType = [
                    'uniform1i',
                    'uniform1i',
    ...
    render(){
        ...
        this.setUniform([
            ...
            1,
            this.nowTime,
            timeScale,       // 時間の経過に対するスケール @@@
            distortionScale, // 歪み係数 @@@
            polar,           // 極座標変換するかどうか @@@
            noiseVisible,    // ノイズを可視化するかどうか @@@
