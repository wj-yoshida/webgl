...
/** ===========================================================================
 * さて、続いては 036 で行ったノイズの表現を、ちょっと質感向上させてみましょう。
 * ここでは、ポイントスプライトという概念を使っています。
 * 頂点を点として描く場合に利用できる、点に対してテクスチャなどのマッピングを行
 * う手法です。単なる点で描くと、どうしても矩形で描かれるので表現力が低くなって
 * しまいますが、ポイントスプライトを活用すると一気に質感が良くなります。
 * ========================================================================= */
...
let pointSize = 8.0;                // ポイントサイズ
let isPointRendering = true;        // 点の頂点を描画するかどうか
let isLineRendering = false;        // ラインの頂点を描画するかどうか
let timeScale = [0.01, 0.01, 0.01]; // 時間の経過に掛ける係数
let distortion = [0.2, 0.2, 0.2];   // 歪ませる量
let translateY = 0.0;               // 全体の位置調整用
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    PANE.addInput({'render point': isPointRendering}, 'render point')
    .on('change', (v) => {isPointRendering = v;});
    PANE.addInput({'render line': isLineRendering}, 'render line')
    .on('change', (v) => {isLineRendering = v;});
    for(let i = 0; i < timeScale.length; ++i){
        PANE.addInput({'time scale': timeScale[i]}, 'time scale', {
            step: 0.005,
            min: 0.005,
            max: 0.5,
        }).on('change', (v) => {timeScale[i] = v;});
        PANE.addInput({'distortion': distortion[i]}, 'distortion', {
            step: 0.01,
            min: 0.0,
            max: 1.0,
        }).on('change', (v) => {distortion[i] = v;});
    }
    PANE.addInput({'translate Y': translateY}, 'translate Y', {
        step: 0.1,
        min: -2.0,
        max: 2.0,
    }).on('change', (v) => {translateY = v;});
...
class WebGLFrame {
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.uniLocation = [
                    ...
                    gl.getUniformLocation(this.program, 'noiseTexture'),   // ノイズテクスチャ
                    gl.getUniformLocation(this.program, 'time'),           // 経過時間
                    gl.getUniformLocation(this.program, 'timeScale'),      // 経過時間に掛ける係数
                    gl.getUniformLocation(this.program, 'distortion'),     // 歪ませる量
                ...
                // ノイズテクスチャ
                return this.createTextureFromFile('./snoise.png');
            })
            .then((texture) => {
    ...
    setup(){
        ...
        // 頂点データ（XZ 平面に敷き詰める）
        const VERTEX_COUNT = 128; // 頂点の個数（効果をわかりやすくするためちょっと減らしてます）
        const VERTEX_WIDTH = 5.0; // 頂点が並ぶ範囲の広さ
        this.position = [];       // 頂点座標
        ...
        for(let i = 0; i < VERTEX_COUNT; ++i){
            ...
            for(let j = 0; j < VERTEX_COUNT; ++j){
                ...
                // カウンタの値からテクスチャ座標を生成
                this.texCoord.push(i / VERTEX_COUNT, j / VERTEX_COUNT);

                // 頂点のインデックス
                if(j > 0){
                    // その列の２つ目以降は直前の頂点と自身を結ぶようにラインを引く
                    let idx = i * VERTEX_COUNT + j;
                    this.index.push(idx - 1, idx);
                }
            }
        }
        this.vbo = [
        ...
        // テクスチャを途中で切り替えるわけではないので、先にバインドしておく
    ...
    render(){
        ...
        this.mMatrix = MAT.translate(this.mMatrix, [0.0, translateY, 0.0], this.mMatrix);
