...
/** ===========================================================================
 * JavaScript には、乱数を生成する仕組みとして Math.random の実装があらかじめ含
 * まれています。一方で、GLSL には言語としてビルトインされている乱数を生成する仕
 * 組みや関数はありません。ですから、シェーダ内で乱数を用いたい場合は、乱数を生
 * 成する仕組みから実装してやらなくてはなりません。
 * ここでは、最もシンプルで簡素な乱数生成器を実装し、単純にノイズをそのまま利用
 * するホワイトノイズのポストエフェクトを行っています。
 * 乱数生成器の種類を２種類定義してあるので、チェックボックスの状態に応じてどち
 * らのロジックを使うのか切り替えながら結果を確認してみましょう。
 * ========================================================================= */
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    PANE.addInput({strength: noiseStrength}, 'strength', {
        step: 0.01,
        min: 0.0,
        max: 1.0,
