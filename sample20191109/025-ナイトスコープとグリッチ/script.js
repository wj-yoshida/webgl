...
/** ===========================================================================
 * ホワイトノイズは、一見すると単なる砂嵐でしかないのであまり使いみちが無いよう
 * に感じた方も多いかもしれません。
 * しかし、単なるランダムな値の上下であっても、使い方次第ではとても印象的な演出
 * になることもあります。
 * ここでは、ナイトスコープシェーダにさらにホワイトノイズを利用したグリッチ演出
 * を組み込んでみましょう。
 * ========================================================================= */
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    PANE.addInput({'glitch threshold': glitchThreshold}, 'glitch threshold', {
        step: 0.01,
        min: 0.01,
        max: 1.0,
    }).on('change', (v) => {glitchThreshold = v;});
    PANE.addInput({'glitch strength': glitchStrength}, 'glitch strength', {
        step: 0.01,
        min: 0.0,
        max: 1.0,
    }).on('change', (v) => {glitchStrength = v;});
    PANE.addInput({'background': backgroundColor}, 'background')
    .on('change', (v) => {
        // 背景色は GLSL では 0.0 ～ 1.0 で表現するので変換する
        backgroundColor = v;
...
class WebGLFrame {
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.postUniLocation = [
                    ...
                    gl.getUniformLocation(this.postProgram, 'glitchThreshold'), // グリッチ発生のしきい値 @@@
                    gl.getUniformLocation(this.postProgram, 'glitchStrength'),  // グリッチ強度 @@@
                    gl.getUniformLocation(this.postProgram, 'time'),            // 時間の経過
                    gl.getUniformLocation(this.postProgram, 'background'),      // 背景色
                ];
                this.postUniType = [
                    'uniform1i',
                    'uniform1i',
    ...
    render(){
        ...
        this.setUniform([
            0,
            noiseTypeOne,         // ノイズのタイプ１かどうか
            noiseStrength,        // ノイズの合成強度
            vignetteScale,        // ヴィネット係数
            sinWave,              // サイン波の周波数係数
            sinStrength,          // サイン波の合成強度
            glitchThreshold,      // グリッチ発生のしきい値 @@@
            glitchStrength,       // グリッチ強度 @@@
            this.nowTime,         // 時間の経過
            backgroundColorFloat, // 背景色
