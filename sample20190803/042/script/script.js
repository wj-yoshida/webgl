...
// = 042 ======================================================================
// CG 的な表現でよく利用するテクニックとしてブラー処理があります。
// ブラーとは要するに「ぼかし」のことで、滲ませるような処理、あるいは光の溢れる
// ような表現など、その使いみちは多岐に渡ります。
// ガウシアンブラーはぼかし処理の代表的な手法のひとつ。シェーダでこれを実装する
// ためには、2pass でぼかしを実現します。生成するフレームバッファが複数になるの
// でちょっと紛らわしいのですが、落ち着いて考えましょう。
// ============================================================================
...
(() => {
    ...
    let framebufferScene; // フレームバッファシーン描画用 @@@
    let framebufferGauss; // フレームバッファガウシアンブラー用 @@@
    ...
    let blurSlider;           // スライダー GUI（ブラー係数） @@@
    let blur = 100;           // ブラー係数 @@@
    ...
    window.addEventListener('load', () => {
        ...
        // ブラー係数 @@@
        blurSlider = new gl3.Gui.Slider('blur', blur, 1.0, 200.0, 1.0);
        blurSlider.add('input', (eve, self) => {
            blur = self.getValue();
        });
        wrapper.append(blurSlider.getElement());
    ...
    function initialize(){
        ...
        // フレームバッファを生成 @@@
        // テクスチャユニット 0： 画像由来のテクスチャ
        // テクスチャユニット 1： シーンを焼いたテクスチャ
        // テクスチャユニット 2： 横方向ブラーの結果を焼いたテクスチャ
        framebufferScene = gl3.createFramebuffer(canvasWidth, canvasHeight, 1);
        framebufferGauss = gl3.createFramebuffer(canvasWidth, canvasHeight, 2);
    ...
    function render(){
        ...
        // - ガウス係数を求める -----------------------------------------------
        // gaussWeight という関数のなかでガウス係数を計算して、結果を配列に格納
        // した状態で返しています。
        // この配列はそのままシェーダのほうへまるっと送ってやり、シェーダ内で色
        // を混ぜ合わせる係数として利用します。
        // --------------------------------------------------------------------
        // ガウシアンブラーの係数を算出 @@@
        let weight = gaussWeight(20.0, blur);
        ...
        // フレームバッファのバインド @@@
        // ここではまずシーンをフレームバッファにそのままレンダリングする
        gl.bindFramebuffer(gl.FRAMEBUFFER, framebufferScene.framebuffer)
        ...
        // - まず横方向に対してのぼかしを行う ---------------------------------
        // この場所までの描画プロセスでは、第一のフレームバッファに対しての通常
        // のレンダリングが終わった状態ですね。
        // 続いては、まずガウシアンブラー用の中継となるフレームバッファに対して
        // まずは横方向にぼかしたシーンを描画します。フレームバッファを切り替え
        // るだけでなく、テクスチャユニットにも注目して考えましょう。
        // --------------------------------------------------------------------
        // 横方向ブラーを掛けた結果を書き込むためにフレームバッファをバインドする
        gl.bindFramebuffer(gl.FRAMEBUFFER, framebufferGauss.framebuffer);
        ...
        postPrg.pushShader([
            ...
            0.0,                         // カーテン（ここでは途中経過なので全面）
            1.0,                         // 補間係数（ここでは途中経過なので補間無し）
            [canvasWidth, canvasHeight], // 画面の解像度
            true,                        // 横方向ブラーのフラグ @@@
            weight                       // ガウス係数の配列 @@@
        ]);
        ...
        // - 最後に縦方向にぼかしを行う ---------------------------------------
        // ブラー用のフレームバッファに横方向にぼかした結果が描き込まれましたの
        // で、続いてはそのシーンに対して、さらに縦方向のぼかしを適用してやりま
        // す。これが、最終的に画面に出力される絵になります。
        // VBO や IBO のバインドは既に済んでいますので、uniform 変数の値の更新
        // だけを正しく行なってやります。
        // --------------------------------------------------------------------
        // フレームバッファのバインドを解除する
