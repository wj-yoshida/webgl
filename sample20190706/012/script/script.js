// = 012 ======================================================================
// 010 で法線を出力したときのことを思い出してみればわかりますが、頂点を回転した
// り動かしたりしたとき、当たり前といえば当たり前ですが、法線はそのままの向きで
// 固定された状態になっています。011 で光の具合いが頂点と一緒に回転していたのは
// 法線がそのままの状態だったからです。
// ライティングを正しく実装するためには、モデル座標変換をベースに、法線を変換す
// るための処理を構築してやらなくてはなりません。
// ============================================================================
...
(() => {
    ...
    function loadShader(){
        prg = gl3.createProgramFromFile(
            ...
            [3, 3, 4],
            ['mvpMatrix', 'normalMatrix'], // 法線変換行列を追加 @@@
            ['matrix4fv', 'matrix4fv'],
            initialize
        );
    }
    ...
    function initialize(){
        ...
        normalMatrix = mat4.identity(mat4.create()); // 法線変換行列 @@@
    ...
    function render(){
        ...
        //順番的にはコードの記述順と、実際の作用が逆になってることに注意！！（かける順番がすごく重要）
        mat4.multiply(vpMatrix, mMatrix, mvpMatrix);
        // - 法線変換用の行列 -------------------------------------------------
        // 法線は、頂点がどちらを向いているのかを表すベクトルでした。
        // つまり、頂点が「モデル座標変換によって動く」とき、一緒に法線も変換し
        // てやらないと、法線の向きが一定のままとなりおかしな状態になります。
        // ですから一般に、ライティングを行う際はモデル座標変換の状態を元に、法
        // 線変換用の行列を別途生成してシェーダに渡してやります。
        // 通常、法線用の行列にはモデル座標変換の「逆転置行列」を用います。
        // --------------------------------------------------------------------
        // 法線変換行列の生成 @@@
        mat4.transpose(mMatrix, normalMatrix);    // 転置
        mat4.inverse(normalMatrix, normalMatrix); // 逆行列
        // uniform 変数をシェーダにプッシュ
        prg.pushShader([
            mvpMatrix,
            normalMatrix // 法線変換行列をシェーダに送る @@@
        ]);
        // ドローコール（描画命令）
