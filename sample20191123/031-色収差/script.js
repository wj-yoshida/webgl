...
/** ===========================================================================
 * フレームバッファを用いたテクニックに色収差があります。色収差というとなにやら
 * 大げさな感じがするかもしれませんが、要するに RGB をずらした表現のことです。
 * 色収差を行うと、どこかブラウン管テレビのような少しアナログな印象になります。
 * ポイントは、あまりに大きく色をずらさないようにすること。以前扱ったモザイクな
 * どと組み合わせたりすることで、より本物のオールドスタイルに近づけることもでき
 * ます。
 * また、色収差の効果をわかりやすくするために、このサンプルでは背景色を完全な黒
 * にしています。全体が明るいシーンでは、色収差の結果がややわかりにくくなること
 * もあるので、状況に応じてパラメータを調整するとよいでしょう。
 * ========================================================================= */

const MAT = new matIV();
const QTN = new qtnIV();

let offset = 0.0025; // オフセットする量 @@@
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.postUniLocation = [
                    ...
                    gl.getUniformLocation(this.postProgram, 'offset'), // オフセットする量 @@@
                ];
                this.postUniType = [
                    'uniform1i',
                    'uniform1f',
