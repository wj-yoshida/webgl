...
/** ===========================================================================
 * モザイク模様のような、一様に解像度の下がった状態だけでなく、同じような概念を
 * 応用することで、グリッチノイズのようなエフェクトを行うこともできるようになり
 * ます。
 * ナイトスコープで行ったような、1px 単位でのテクスチャ座標ズレとはまた少し質感
 * が違った、独特な表現を生み出すことができます。
 * ========================================================================= */

const MAT = new matIV();
const QTN = new qtnIV();

let blockSize = 8.0;   // ブロックサイズ（1.0 ～ 64.0）
let threshold = 0.05;  // グリッチ発生のしきい値（0.001 ~ 0.1） @@@
let slideWidth = 0.05; // グリッチ発生時のズレ幅（0.001 ~ 0.1） @@@
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    PANE.addInput({'block size': blockSize}, 'block size', {
        step: 0.01,
        min: 1.0,
        max: 64.0,
    }).on('change', (v) => {blockSize = v;});
    PANE.addInput({'threshold': threshold}, 'threshold', {
        step: 0.001,
        min: 0.001,
        max: 0.1,
    }).on('change', (v) => {threshold = v;});
    PANE.addInput({'slide width': slideWidth}, 'slide width', {
        step: 0.001,
        min: 0.001,
        max: 0.1,
    }).on('change', (v) => {slideWidth = v;});
...
class WebGLFrame {
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.postUniLocation = [
                    ...
                    gl.getUniformLocation(this.postProgram, 'blockSize'),
                    gl.getUniformLocation(this.postProgram, 'threshold'),
                    gl.getUniformLocation(this.postProgram, 'slideWidth'),
    ...
    render(){
        ...
        this.setUniform([
            ...
            blockSize,    // ブロックサイズ
            threshold,    // グリッチ発生のしきい値 @@@
            slideWidth,   // グリッチ発生時のズレ幅 @@@
            this.nowTime, // 時間の経過
