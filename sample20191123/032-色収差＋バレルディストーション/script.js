...
/** ===========================================================================
 * 色収差はそれ単体でも十分に活用できるテクニックですが、より自然な印象に仕上げ
 * るために、バレルディストーションと組み合わされることも多いです。
 * バレルディストーションの「バレル」とは樽のことです。
 * まるで樽のようにスクリーン全体を歪ませる処理のことを、その影響が樽のような形
 * に見えることからバレルディストーションと呼ばれます。
 * バレルディストーションは、要するに画面の外側に近づくほど歪む量を増やしていけ
 * ばいいので、画面の中心からの距離を測った結果をうまく活用します。
 * ========================================================================= */

const MAT = new matIV();
const QTN = new qtnIV();

let offset = 0.01;    // 色収差のオフセットする量
let coordScale = 0.8;   // バレルディストーションの影響範囲 @@@
let barrelScale = 0.5; // バレルディストーションのスケール @@@
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    PANE.addInput({'offset': offset}, 'offset', {
        step: 0.0001,
        min: 0.0,
        max: 0.01,
    }).on('change', (v) => {offset = v;});
    PANE.addInput({'coord scale': coordScale}, 'coord scale', {
        step: 0.01,
        min: 0.0,
        max: 1.0,
    }).on('change', (v) => {coordScale = v;});
    PANE.addInput({'barrel scale': barrelScale}, 'barrel scale', {
        step: 0.001,
        min: 0.0,
        max: 0.1,
    }).on('change', (v) => {barrelScale = v;});
...
class WebGLFrame {
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.postUniLocation = [
                    ...
                    gl.getUniformLocation(this.postProgram, 'offset'),      // 色収差のオフセットする量
                    gl.getUniformLocation(this.postProgram, 'coordScale'),  // バレルディストーションの影響範囲 @@@
                    gl.getUniformLocation(this.postProgram, 'barrelScale'), // バレルディストーションのスケール @@@
                ];
                this.postUniType = [
                    'uniform1i',
                    'uniform1f',
    ...
    render(){
        ...
        this.setUniform([
            0,
            offset,      // 色収差のオフセットする量
            coordScale,  // バレルディストーションの影響範囲 @@@
            barrelScale, // バレルディストーションのスケール @@@
