...
// = 031 ======================================================================
// キューブ環境マッピングは、反射ベクトルを用いて正六面体に対してサンプリングを
// 行う手法です。ここで使われている反射ベクトルの算出方法をほんの少し変えてやる
// と、また違った効果が得られます。
// より具体的には、反射ではなく屈折を再現すれば、ガラスなどの透明な物体のような
// 質感を再現することもできます。
// 屈折率はウェブで検索すれば比較的簡単に一覧が見つかりますので、そこで出てくる
// 屈折率で 1.0 を割って、それを目安の値として使います。
// 例（屈折率が 1.5 なら）： 1.0 / 1.5 => 約 0.67
// ============================================================================
...
(() => {
    ...
    let refractSlider;        // スライダー GUI（屈折率） @@@
    let refract = 0.5;        // 屈折率 @@@
    ...
    window.addEventListener('load', () => {
        ...
        // 屈折率 @@@
        refractSlider = new gl3.Gui.Slider('refraction', refract, 0.01, 1.0, 0.01);
        refractSlider.add('input', (eve, self) => {
            refract = self.getValue();
        });
        wrapper.append(refractSlider.getElement());
    ...
    function loadShader(){
        ...
        prg = gl3.createProgramFromFile(
            ...
            [
                ...
                'cubeTexture',
                'refractIndex', // 屈折率 @@@
                'refraction'    // 屈折させるかどうか @@@
            ],
            ['matrix4fv', 'matrix4fv', 'matrix4fv', '3fv', '1i', '1f', '1i'],
            loadTexture
        );
    }
