...
vec2 map(vec3 p) {
    ...
    float sphere = rounding(sdCappedCone( q, 0.4, 0.3, 0.05 ), 0.2);//角丸の円錐
    ...
    vec2 tile2 = fract(p.xz * 9.0) * 3.0 - 2.5;
    ...
    vec2 d = min2(vec2(plane + tileHeight * 0.02, MAT_FLOOR), vec2(sphere + tileHeight2, MAT_BALL));
