...
/** ===========================================================================
 * これまで見てきたサンプルでもそうですが、本当にちょっとしたことで、劇的に描画
 * 結果が変化するのはシェーダコーディングの醍醐味だと言えます。
 * ここでは、それこそ本当にシンプルな方法を使って、テクスチャ座標のクランプ表現
 * を行ってみましょう。
 * まず最初に描画結果を見てから、どのような処理を行えば同様の描画結果が得られる
 * のか、考えてみるのもいいかもしれません。
 * ========================================================================= */
...
let clampValue = 0.5; // テクスチャ座標のクランプ値 @@@
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    PANE.addInput({'clamp value': clampValue}, 'clamp value', {
        step: 0.01,
        min: 0.0,
        max: 1.0,
    }).on('change', (v) => {clampValue = v;});
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.postUniLocation = [
                    ...
                    gl.getUniformLocation(this.postProgram, 'clampValue'), // テクスチャ座標のクランプ値 @@@
                ];
                this.postUniType = [
                    'uniform1i',
                    'uniform1f',
