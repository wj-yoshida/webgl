...
// = 043 ======================================================================
// WebGL 2.0 は、現状はまだまだモバイル端末を中心に、動作しない場合があるものの、
// PC ベースでは、ほぼ動くような状況になってきています。
// WebGL 2.0 で追加された機能は、ややマイナーなものが多いのですが、そのなかでも
// 特にインパクトの強い transform feedback をやってみます。
// transform feedback は、VBO を直接シェーダで編集してしまうという機能を持ってい
// て、GPU（シェーダ）の力を借りて VBO を高速に更新します。これにより、GPGPU と
// 一般に呼ばれている汎用計算を、GPU を使って行うことができるようになります。
// ============================================================================

(() => {
    // constant @@@
    const WIDTH  = 512; //横方向に並ぶ頂点の数
    const HEIGHT = 512;//縦方向に並ぶ頂点の数
    ...
    window.addEventListener('load', () => {
        ...
        //自分でwebgl2.0を実行する場合はcanvasをとってきて、 let gl = canvas.getContext("webgl2");
        // glcubic の初期化
    ...
    function loadShader(){
        ...
        tfPrg = gl3.createProgramFromFileTF(
            './shader/transform-feedback.vert',
            './shader/transform-feedback.frag',
            ['position', 'velocity', 'color'],
            [3, 3, 4],
            [
                'time',
    ...
    function render(){
        ...
        //偶数と奇数
        ++count;
        let countIndex = count % 2;//０か１かを繰り返す
        let invertIndex = 1 - countIndex;//１か０をくり返す

        // トランスフォーム・フィードバックの開始 =============================
        tfPrg.useProgram();
        // *1 setAttributeでバインドしたVBO
        tfPrg.setAttribute(VBOArray[countIndex]);
        // VBO の設定
        //　*2 baffer base　でバイんどしているVBO
        //*1と*2はフレームごとに切り替わる
